# Civil Case Management Repository

This repository contains case management tools and documentation for civil court cases.

## 📅 Quick Links - Court Dates & Deadlines

### 🔥 **[UPCOMING DATES SUMMARY](UPCOMING_DATES_SUMMARY.md)** ⭐
**START HERE** - Quick answer to "When are my court dates?" with immediate action steps.

### Additional Resources:
- **[Court Calendar](Court_Calendar.md)** - View all upcoming court dates and deadlines
- **[Case Status Tracker](Case_Status_Tracker.md)** - Track status of all active and closed cases
- **[How to Check Court Dates](How_to_Check_Court_Dates.md)** - Step-by-step guide to checking court dates online, by phone, or in person

## 📁 Active Cases

### Draft #34703073 - Edith
- **Status:** Pre-Filing Preparation
- **Folder:** `/Draft_34703073_Edith/`
- **Type:** Property/Security Deposit Dispute

### Draft #34703074 - Concepts 26
- **Status:** Pre-Filing Preparation
- **Folder:** `/Draft_34703074_Concepts26/`
- **Type:** Property Management Dispute

### MCRO 62-CO-25-3554
- **Status:** Active - Filed July 17, 2025
- **Court:** Ramsey County Conciliation Court
- **Type:** Small Claims

## 🛠️ Case Management Tools

### Court Date Tracking Scripts
- **[Show_Court_Dates.ps1](Show_Court_Dates.ps1)** - PowerShell script to display upcoming court dates (Windows)
- **[show_court_dates.sh](show_court_dates.sh)** - Bash script to display upcoming court dates (Linux/Mac)

Run these scripts anytime to quickly see your court date status!

### Conversation & Communication Tools
- **[Master Conversation Organizer](Master_Conversation_Organizer.ps1)** - PowerShell script to organize conversations
- **[Conversation Transcription Tool](Conversation_Transcription_Tool.ps1)** - Tool for transcribing conversations
- **[Email Extractor](email_extractor.py)** - Python script to extract emails from .mbox files

### Document Templates
Each case folder contains:
- Conversation templates
- Email log templates
- Demand letter templates
- Case summaries

## 📖 Resources

- Minnesota Courts: https://www.mncourts.gov/
- Court Records Search: https://pa.courts.state.mn.us/CaseSearch
- Self-Help Center: https://www.mncourts.gov/Help-Topics/Self-Help-Centers.aspx

## 🔐 Privacy Note

This repository contains personal legal case information. Keep secure and do not share publicly.

---

**Repository Owner:** Ira Toles  
**Last Updated:** January 2025
